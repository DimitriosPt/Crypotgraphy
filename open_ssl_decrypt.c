...
10 September 2019
The following code decrypts a text file encrypted using openSSL. It runs on the command line
and takes a password, an input file (what you wish to decrypt), 
and an output file for writing the plaintext to.
*/
...
int main(int argc, const char* argv[]) {
    ...
    EVP_DigestUpdate(mdctx, argv[1], strlen(argv[1])); //take password and make SHA hash
    EVP_DigestFinal_ex(mdctx, key, NULL); //take the hash and make a key
    EVP_MD_CTX_destroy(mdctx);	//destroy the hash
    ...
    fread(iv, 1, BLK_LEN, src);	//read iv from source
